...
export default function PlasmaShader({ className = '' }) {
  ...
  useEffect(() => {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.warn('Plasma WebGL Program Link Error:', gl.getProgramInfoLog(program));
      return;
    }
    ...
    gl.uniform4f(uShape, 1.50, 0.48, 0.50, 0.00);
    gl.uniform4f(uSurface, 2.40, 0.92, -0.50, 1.00);
    gl.uniform4f(uFinish, 3.03687, 0.61, 0.016, 0.35); // hue 174 deg = 3.03687 rad
    gl.uniform4f(uTransform, 7.0, 0.00, 0.16, 0.0);
    gl.uniform4f(uSpace, 0.00, 0.00, 0.00, 0.00);
    gl.uniform4f(uCursor, 0.0, 4.0, 0.65, 0.30);
    ...
    const render = (now) => {
      ...
      gl.uniform4f(uScene, canvas.width, canvas.height, seconds * 0.86, 4.0);
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 size-full overflow-hidden pointer-events-none select-none z-0 ${className}`}
    >
